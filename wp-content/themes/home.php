        ...
        <div class="full-bleed-container row">
            ...
            <?php foreach ($featured_posts as $post) : ?>
                ...
                <div
                    ...
                    <div class="info">
                        <a href="<?php echo esc_url( get_category_link($categories[0]->cat_ID) ); ?>"><span class="category text-content"><?php echo $categories[0]->name; ?></span></a> - <span
                            class="publish-date"><?php echo mysql2date('j M Y', $post->post_date); ?></span>
                        <a class="text-content read-more" href="<?php echo esc_url( get_permalink($post->ID) ); ?>">Read more</a>
                    </div>
                </div>
				<?php 
					$bleed_img_url = wp_get_attachment_image_src(
                         get_post_thumbnail_id($post->ID), 'home-slider-img'
                    ); 
				?>
				<?php if(!empty($bleed_img_url)): ?>
					<div class="full-bleed-image"
						 style="background:url(<?php print $bleed_img_url[0]  ?>) 50% 50% / cover no-repeat;"></div>	
				<?php endif; ?>
                <?php $i++ ?>
            <?php endforeach; ?>
                    ...
                    <li class="carouse-item col-lg-4 col-sm-4" data-id=<?php echo $i; ?>>
                        <div class="image">
                            <div class="round-mask">
								<?php
									// "home-round" is 55*55 but looks badly
								    $carousel_img_url = wp_get_attachment_image_src(
										 get_post_thumbnail_id($post->ID), "thumbnail"
									);
								?>
								<?php if(!empty($carousel_img_url)): ?>
									<img src="<?php echo $carousel_img_url[0] ?>" alt=""/>
								<?php endif; ?>
                            </div>
                        ...
                        <div class="info">
                            <a href="<?php echo esc_url( get_category_link($categories[0]->cat_ID) ); ?>"><span class="category"><?php echo $categories[0]->name; ?></span></a> - <span
                                class="publish-date"><?php echo mysql2date('j M Y', $post->post_date); ?></span>
                        </div>
                    </li>
                    <?php $i++ ?>
                    ...
                    <article id="post-<?php echo $i ?>"
                        ...
                        <a href="<?php echo esc_url( get_permalink($post->ID) ); ?>">
                            ...
                            <?php if (!empty(get_post_thumbnail_id($post->ID))): ?>
								<?php
								    $hp_post_img_url = wp_get_attachment_image_src(
										 get_post_thumbnail_id($post->ID), "home-post-header-bg"
									);
								?>
                                <header class="post-header"
                                        style="background: url(<?php echo $hp_post_img_url[0] ?>) 50% 50% / cover no-repeat; background-color:<?php echo $catColor[0];?>;"></header>
                            <?php else: ?>
                            ...
                            <div class="text-wrapper">
                                ...
                                <div class="info">
                                    <a href="<?php echo esc_url( get_category_link($categories[0]->cat_ID) ); ?>"><span class="category"><?php echo $categories[0]->name; ?></span></a>
