...
function zocdoc_scripts() {
	...
	wp_enqueue_style('provider-style', get_template_directory_uri() . '/styles/output/desktop/styles/provider-style.css');
...
function default_page_template( $template ) {
    ...
    if ( is_singular( 'page' )  ) {
        $default_template = locate_template( array( 'page-evolutions.php' ) );
        if ( '' != $default_template ) {
