...
function _nc_clone_featured_image( $args, $media_item ) {
    ...
    $args[4] = array(
        ...
        'image_alt' => get_post_meta( $media_item->ID, '_wp_attachment_image_alt')
    );
...
function nc_check_draft_query( $posts, &$query ) {
    ...
    $query_token = $_GET[NC_QP_PREVIEW_TOKEN];
    if ((isset($query_token)) && !empty($query_token) && ($query_token === $nc_token)) {
        $query->_draft_post = $posts;
        add_filter( 'the_posts', 'nc_show_draft_post', null, 2 );
    }

    return $posts;
}

function nc_show_draft_post( $posts, &$query ) {
    remove_filter( 'the_posts', 'nc_show_draft_post', null, 2 );
...
function nc_allow_protected_meta_edit( $allcaps, $cap, $args ) {
    if (in_array($args[0], array( 'edit_post_meta', 'add_post_meta' ))) {
        if ((isset($args[2]) && current_user_can('edit_post', $args[2])) && ((isset($args[3] ) && $args[3] !== '') && strpos($args[3], '_nc-') === 0 )) {
            $allcaps[$args[0]] = true;
        }
    }
